...
`Данный проект разрабатывается в рамках академических курсов (Курсы «Игровая экономика и балансировка игр», «Проектная документация»).`<br>
`Основной упор сделан на математическую модель, расчет темпа игры и окупаемости карт (ROI), а не на визуальную составляющую. Система спроектирована как гибко настраиваемая через внешние конфигурационные файлы.`

---

## 📋 Оглавление

1. [Концепция](#2-концепция)
2. [Математика и баланс](#3-математика-и-баланс)
3. [Механики и правила](#4-механики-и-правила)
4. [Карты и описание](#5-карты-и-описание)
5. [Технологический стек](#6-технологический-стек)
6. [Конфигурация и настройка](#7-конфигурация-и-настройка)
7. [Прозрачность и логирование](#8-прозрачность-и-логирование)
8. [Руководство по запуску](#9-руководство-по-запуску)
9. [Лицензия и контакты](#10-лицензия-и-контакты)
...
**Игроки**: 2–4 (оптимально 3–4 для баланса Red-карт)<br>
**Цель**: Первым накопить ≥ WinTarget монет (по умолчанию: 100)<br>

### 2.1. Нарративное описание
...
### 2.2. Основные параметры сессии

| Параметр              | Значение   | Описание                            |
|-----------------------|------------|-------------------------------------|
| **Игроки**            | 2–4        | Оптимально: 3–4 игрока              |
| **Стартовый капитал** | 5–10 корма | Случайное значение в диапазоне      |
| **Карт на рынке**     | N+1        | Где N — количество игроков          |
| **Целевая победа**    | 100 корма  | Настраивается в `appsettings.json`  |
| **Ежедневный доход**  | +1 корм    | Гарантированно в конце каждого хода |

---

## 3. Математика и баланс

### 3.1. Якорные значения и формулы

> **Якорные значения**: `DailyIncome = 1`, `P(Color) = 0.25`, `Target Payback = 3-5 ходов`<br>
> **Формула**: `Payback = Cost / (Yield × P(Color) × P(Activation))`, где для прямых эффектов `P(Activation)=1`

### 3.2. Таблица карт и расчёты
...
\* `P(Appear) = Weight / ΣWeight`, где ΣWeight = 560<br>
\** `ROI = (Yield × 0.25 × 10) - Cost` — ожидаемая прибыль за 10 раундов<br>
\*** Для Red: `Yield = X × (N_players - 1)`, при 4 игроках: ×3<br>
\**** Оценка через симуляцию: средняя ценность украденной карты ≈ 3 корма

### 3.3. Выводы по балансу
...
### 3.5. Параметры баланса (appsettings.json)
...
### 4.1. Структура игрового круга

```mermaid
flowchart TD
Start[📅 Начало круга] --> Phase1[1️⃣ Фаза события]

    subgraph EventPhase [Подготовка фазы]
        P1a[Ротация очередности]
        P1b[Обновление рынка N+1]
        P1c[Генерация ActiveColor]
    end

    Phase1 --> P1a & P1b & P1c
    EventPhase --> Player1[2️⃣ Ход Игрока 1]
    
    subgraph Actions1 [Действия Игрока 1]
        Act1[Активация карт]
        Buy1[Покупка 1 карты]
        Inc1[+1 корм доход]
    end

    Player1 --> Act1 & Buy1 & Inc1
    Actions1 --> Check1{Проверка победы}
    
    Check1 -- Balance >= 100 --> Win[🏆 ПОБЕДА]
    Check1 -- Balance < 100 --> Player2[3️⃣ Ход Игрока 2]
    
    subgraph Actions2 [Действия Игрока 2]
        Act2[Активация карт]
        Buy2[Покупка 1 карты]
        Inc2[+1 корм доход]
    end

    Player2 --> Act2 & Buy2 & Inc2
    Actions2 --> Check2{Проверка победы}
    
    Check2 -- Balance >= 100 --> Win
    Check2 -- Balance < 100 --> PlayerN[... Ход Игрока N]
    
    subgraph ActionsN [Действия Игрока N]
        ActN[Активация карт]
        BuyN[Покупка 1 карты]
        IncN[+1 корм доход]
    end

    PlayerN --> ActN & BuyN & IncN
    ActionsN --> CheckN{Проверка победы}
    
    CheckN -- Balance >= 100 --> Win
    CheckN -- Balance < 100 --> Start
    
    style Start fill:#4CAF50,color:white
    style Win fill:#2196F3,color:white
    style Check1 fill:#9C27B0,color:white
    style Check2 fill:#9C27B0,color:white
    style CheckN fill:#9C27B0,color:white
...
### 4.3. Правила активации карт по цветам

| Цвет          | Сектор           | Условие активации       | Кто получает эффект                 |
|---------------|------------------|-------------------------|-------------------------------------|
| 🔵 **Blue**   | Производственный | `ActiveColor == Blue`   | **Все игроки**                      |
...
### 4.5. Обработка случайных событий
...
| **Deployment** | http://relaxerr-games.ru    | Публикация билда                             |
...
### 7.1. Общие настройки (appsettings.json)
...
| **C**   | **Effect**      | Команда DSL-движка (см. раздел 7.3)                      | `GET 3`             |
...
### 7.3. Синтаксис эффектов (DSL Engine)
...
### 7.5. Визуальные ресурсы

Все изображения должны находиться в папке: `wwwroot/images/cards/`.<br>
В колонке Icon указывайте полное имя файла (например, `wheat.png`).<br>
Если файл не найден, в консоли сервера появится [WARN], а на карте отобразится заглушка.

### 7.6. Типичные ошибки при настройке
...
## 8. Прозрачность и логирование

Для исключения спорных ситуаций система ведёт Лог событий, в который записываются:
...
Проект разработан в учебных целях. Для вопросов по механике, балансу или интеграции обращайтесь к автору проекта (tg: https://t.me/relaxerr_teech).<br>
**Использование любых файлов этого проекта только с указанием ссылки на данный репозиторий**: https://github.com/RelaxerR/TinyCityCardGame_online
